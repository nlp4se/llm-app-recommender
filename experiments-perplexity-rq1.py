...
def run_command(k, search, output_dir, n=10, model="sonar", sleep=10):
    """Run the search command with given parameters."""
    cmd = [
        "python", "-m", "code.llm.perplexity.search_perplexity_rq1",
        "--output", output_dir,
        "--k", str(k),
        "--search", search,
        "--n", str(n),
...
def main():
    ...
    base_output_dir = "./data/output/features/rq1/perplexity"
