...
def run_command(k, category, output_dir, n=1, model="gpt-4o-search-preview", sleep=10):
    """Run the search command with given parameters."""
    cmd = [
        "python", "-m", "code.llm.openai.search_openai_rq2",
        "--output", output_dir,
        "--k", str(k),
        "--category", category,
        "--n", str(n),
        "--model", model,
        "--sleep", str(sleep),
        "--criteria-folder", f"data/output/search/rq1/openai/k{str(k)}_{category.replace(' ', '_')}"
    ]
...
def main():
    ...
    base_output_dir = "./data/output/search/rq2/openai"
