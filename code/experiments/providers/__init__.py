...
def get_client(spec: ModelSpec, *, web_search: bool = False) -> LLMClient:
    if spec.provider == Provider.OPENAI:
        from code.experiments.providers.openai import OpenAIClient

        return OpenAIClient(spec.model_id, web_search=web_search)
    if spec.provider == Provider.GEMINI:
        from code.experiments.providers.gemini import GeminiClient

        return GeminiClient(spec.model_id, web_search=web_search)
    if spec.provider == Provider.ANTHROPIC:
        from code.experiments.providers.anthropic import AnthropicClient

        return AnthropicClient(spec.model_id, web_search=web_search)
    if spec.provider == Provider.MISTRAL:
        from code.experiments.providers.mistral import MistralClient

        return MistralClient(spec.model_id, web_search=web_search)
    if spec.provider == Provider.PERPLEXITY:
        from code.experiments.providers.perplexity import PerplexityClient

        return PerplexityClient(spec.model_id, web_search=web_search)
    if spec.provider == Provider.DEEPSEEK:
        from code.experiments.providers.deepseek import DeepSeekClient

        return DeepSeekClient(spec.model_id, web_search=web_search)
