        ...
        for index, row in criteria_df.iterrows():
            ...
            for i in range(n):
                ...
                try:
                    ...
                    logger.error(f"Request error in run {i+1} for criteria '{search}': {str(e)}")
...
def main(output_folder: str, k: int, input_csv: str, search: str, n: int = 1, model: str = "sonar", sleep_time: int = 10):
    searchPerplexity = PerplexitySearch(model)
    searchPerplexity.run_prompt(output_folder, k, input_csv, search, n, sleep_time)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run prompts with Perplexity API based on a CSV input.')
