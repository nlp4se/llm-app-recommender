...
def run_command(k, category, output_dir, n=1, model="gemini-2.0-flash", sleep=10):
    """Run the search command with given parameters."""
    cmd = [
        "python", "-m", "code.llm.google.search_gemini_rq2",
        "--output", output_dir,
        "--k", str(k),
        "--category", category,
        "--n", str(n),
        "--model", model,
        "--sleep", str(sleep),
        "--system-prompt", "data/input/prompts/system-prompt-output-rq2.txt",
        "--criteria-folder", f"data/output/search/rq1/gemini/k{str(k)}_{category.replace(' ', '_')}"
    ]
...
def main():
    ...
    base_output_dir = "./data/output/search/rq2/gemini"
